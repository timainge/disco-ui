...
import { Plus, Folder, Search, MoreVertical, Calendar, Mail, FileText, Globe, HardDrive, CheckCircle2, Loader2, ArrowRight, ArrowLeft } from 'lucide-react';
...
function NewCaseWizard({ onCancel, onComplete }: { onCancel: () => void, onComplete: () => void }) {
...
function SourceCard({ icon, title, selected, onClick }: { icon: React.ReactNode, title: string, selected: boolean, onClick: () => void }) {
  return (
    <div 
      onClick={onClick}
...
function Checkbox({ label, icon, checked, onChange }: { label: string, icon?: React.ReactNode, checked: boolean, onChange: () => void }) {
  return (
